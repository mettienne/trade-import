...
def index():
    ...
    for collection in collections:
        run('mongo invoice --eval "db.{}.ensureIndex({{ key: 1 }}, {{ unique: true }})"'.format(
            collection))
...
def start():
    print(_yellow('>>> starting {}'.format(_fn())))
    with cd(env.app_path):
        run('sudo supervisorctl reload && sleep 3 && sudo supervisorctl restart edi_ftp_stage:* && sudo supervisorctl restart edi_ftp_prod:*')


def sync():
    print(_yellow('>>> starting {}'.format(_fn())))
    local(
        'rsync -vaz {}@{}:/export {}'.format(env.user, env.host, config.rsync_dir))
...
def clone():
    with settings(warn_only=True):
        with cd(config.apps_dir):
            run('git clone -q --depth 1 {} {} || true'.format(env.git_clone,
                                                              env.app_name))
...
def install_requirements():
    ...
    print(_yellow('>>> starting {}'.format(_fn())))
    virtualenv(
        'pip install -q --upgrade -r {}/requirements.txt'.format(env.app_path))
